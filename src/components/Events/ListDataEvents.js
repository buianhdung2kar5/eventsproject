...
export default function ListDataEvents({ data }) {
  ...
  const dataPerPage = dataEvent?.slice(start, end)
  const totalPage =
    dataEvent?.length > itemsPerPage
      ? Math.ceil(dataEvent?.length / itemsPerPage)
      : 1
  const viewDetail = (id) => {
    ...
    <>
        ...
        {dataPerPage?.map((item) => {
          return (
            ...
            >
              ...
              <div className="w-full h-[44%] pb-2 relative overflow-hidden">
                <img
                  src={process.env.PUBLIC_URL + `${item.img}`}
                  className="w-full h-full object-cover border-0 rounded-tr-lg rounded-tl-lg whitespace-nowrap"
                  alt={item.name}
                />
              ...
              <div className="flex flex-col gap-1">
                <div className="flex items-center relative p-2">
                  <p className="absolute left-2 bg-[#EFF1CF] border rounded-[10px] font-medium px-2 py-[2px] text-[12px] whitespace-nowrap">
                    {item.category}
                  </p>
      ...
      <div className="flex gap-4 items-center justify-center w-full">
        {[...Array(totalPage)]?.map((_, index) => {
          return (
            <button key={index} onClick={(index) => handleChangePage}>
              {index + 1}
            </button>
          )
        })}
